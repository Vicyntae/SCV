ScriptName SCVFrenzyLevel02 Extends ActiveMagicEffect
...
String Property DebugName
  String Function Get()
    Return "[SCV FrenzyLvl02: " + MyActorName + "] "
  EndFunction
EndProperty
Int DMID = 7
...
Event OnEffectStart(Actor akTarget, Actor akCaster)
  MyActor = akTarget
  ActorData = SCVLib.getTargetData(akTarget)
...
Event OnUpdate()
  ...
  If Chance < Severity / 2
    ...
    If Fullness < Min
      ...
      If (Eaten < MealSize / 3) && (Severity > 150)
        ;If !MyActor.IsInCombat()
          SCV_AIFindOVPreySpell01a.Cast(MyActor)
        ;EndIf
        Severity += Math.Ceiling(Min - Eaten)
      ElseIf Eaten < (MealSize / 2) && (Severity < 150) && (Severity > 100)
        Severity += Math.Ceiling(Min - Eaten)
        If !MyActor.IsInCombat()
          SCL_AIFindFoodSpell01b.Cast(MyActor)
        EndIf
      Else
        Severity -= Math.Ceiling(Eaten / 2)
      EndIf
    EndIf
  EndIf
  RegisterForModEvent("SCV_InsertEvent", "OnPreyEaten")
...
Event OnPreyEaten(Form akPred, Form akPrey, Int aiItemType, Bool Friendly)
  If akPred == MyActor && akPrey as Actor
    If aiItemType == 1 || aiItemType == 2
      ...
      EndIf
      If MyActor.HasMagicEffect(SCL_AIFindFoodEffect01a) || MyActor.HasMagicEffect(SCL_AIFindFoodEffect01b)
...
Event OnObjectEquipped(Form akBaseObject, ObjectReference akReference)
  If akBaseObject as Potion || akBaseObject as Ingredient
    If JMap.getInt(SCVLib.getItemDataEntry(akBaseObject), "STIsNotFood") == 0
      Severity -= Math.Floor(SCVLib.genDigestValue(akBaseObject) / 2)
